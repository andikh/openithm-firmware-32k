...
AirSensor::AirSensor(int requiredSamples) : thresholds{ 10000, 10000, 10000, 10000, 10000, 10000 }, samplesToAcquire(requiredSamples), calibrated{ 0, 0, 0, 0, 0, 0 }, highestthresholds {10000, 10000, 10000, 10000, 10000, 10000}
{
  EEPROM.get(66, analogSensitivity);
  if (analogSensitivity == 0 || analogSensitivity >= 100)
    setAnalogSensitivity(DEFAULT_SENSITIVITY);
  for(int i = 0 ; i < 6; i++) pinMode(ir_sensor_pins[i], INPUT);
  calibrateAnalogv2();
}
...
void AirSensor::calibrateAnalogv2() {
  // wait 3 second for all light to turn on
  for (CRGB& led : leds)
    led = CRGB::Orange;
    FastLED.show();
    delay(2000);
  ...
  for(int i = 0 ; i < samplesToAcquire /3; i++) {
    touchboard -> scan();
      ...
      for (int sensor = 0; sensor < 6; sensor++)
    {
      ...
      if (value < highestthresholds[sensor])
        highestthresholds[sensor] = value;
      turnOffLight();
    }
 ...
 // delay(2000);
  ...
  for (int rotation = 0; rotation < 4; rotation++) {
    for (CRGB& led : leds) {
      led = (rotation%2 != 0 ?  CRGB::Orange : CRGB::Red);
    }
    FastLED.show();
  ...
  for(int sensor = 0; sensor < 6; sensor++) {
    ...
    thresholds[sensor] = (highestthresholds[sensor] - thresholds[sensor]) * (analogSensitivity / 100.0f) + thresholds[sensor];
  }
}
...
bool AirSensor::getSensorStateV2(int sensor) {
  int value = getValue(sensor, true);
...
uint8_t AirSensor::getSensorReadingsV2()
{
  uint8_t reading = 0;
  
  for (int i = 0; i < 6; i++)
  {
    reading |= ((int)getSensorStateV2(i) << i);
  }
